...
public class HapticsController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        for (int i = 0; i < _hapticReceivers.Length; i++)
        {
            if (_hapticReceivers[i].gameObject.activeInHierarchy)
            {
                ...
                {
                    for (int j = 0; j <= contactHits.Length / 2; j++)
                    {
                        var randomSel = Random.Range(0, contactHits.Length);
                        contactPoints.Add(contactHits[randomSel].point);
                    }
                }
            }
        }
        ...
        foreach (var pointToEmit in pointsToEmit)
        {
            ...
            var amControlPoint = new AmplitudeModulationControlPoint(deviceSpacePosition, 1f);

            // amControlPoint.setFrequency(147.0f * (float)Units.hertz);
    ...
    List<UnityEngine.Vector3> ChoosePointsToEmit(List<UnityEngine.Vector3> contactPoints)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < _maxControlPoints; i++)
            {
